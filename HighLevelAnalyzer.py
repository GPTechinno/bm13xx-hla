...
def get_reg_name(asic: str, reg_add: int) -> str:
...
def get_core_reg_name(asic: str, reg_id: int) -> str:
...
class Hla(HighLevelAnalyzer):
    ...
    bm_asic = ChoicesSetting(['BM1370', 'BM1368', 'BM1366', 'BM1362', 'BM1397'], label='ASIC')
    bm_clkifreq = NumberSetting(label='CLKI frequency [MHz]', min_value=25, max_value=100)
    bm_ext = ChoicesSetting(['enable', 'disable'], label='Extended Frame')
    ...
    result_types = {
        ...
        'work': {
            'format': 'Work job_id#{{data.job_id}} midstates_cnt#{{data.midstate}} nbits={{data.nbits}} ntime={{data.ntime}} CRC={{data.crc}}'
        },
        'nonce': {
            'format': 'Nonce {{data.nonce}} job_id#{{data.job_id}} small_core_id#{{data.small_core_id}} CRC={{data.crc}}'
        },
        'nonce_ext': {
            'format': 'Nonce {{data.nonce}} job_id#{{data.job_id}} small_core_id#{{data.small_core_id}} version_bits={{data.version_bits}} CRC={{data.crc}}'
        },
        'unknown': {
    ...
    def __init__(self):
        ...
        self._command: str = ""
        # datas
        self._core_small_core_cnt: int = 16 if self.bm_asic == "BM1370" or self.bm_asic == "BM1368" else 8 if self.bm_asic == "BM1366" or self.bm_asic == "BM1362" else 4
        self._core_bits: int = 8 if self.bm_asic == "BM1397" else 7
        self._chipadd: int = 0
        self._regadd: int = 0
        self._regval: int = 0
        # nonce
        self._nonce: int = 0
        self._small_core_id: int = 0
    ...
    def decode(self, frame: AnalyzerFrame):
        ...
        if self._command == "respond":
            ...
            if self._byte_pos == (1 + preamble_offset) and self._frame_type == "VIL":
                # VIL
                self._frame_len = raw
                if self.bm_ext == "enable" and self._command == "work":
                    self._frame_len += 32
            if self._command == "set_chipadd":
                ...
                if self._byte_pos == 1 + preamble_offset + self._vil_offset:
                    self._job_id = (raw >> 3) if self.bm_ext == "enable" else (raw >> 2) # validated
                elif self._byte_pos == 2 + preamble_offset + self._vil_offset:
                    self._midstates = raw
                elif self._byte_pos == 3 + preamble_offset + self._vil_offset:
        ...
        if self._byte_pos == self._frame_len - 1 + preamble_offset:
            ...
            if self._command == "respond" and raw & 0b10000000 == 0b10000000:
                core_small_core_mask = self._core_small_core_cnt - 1
                core_small_core_bits = bin(core_small_core_mask).count('1')
                chunk = (self._chipadd << 8) | self._regadd
                self._job_id = (chunk >> core_small_core_bits) & 0b11111 # validated
                core_id = self._nonce >> (32 - self._core_bits)
                if self.bm_ext == "enable":
                    self._command = "nonce_ext"
                    self._small_core_id = self._regadd & core_small_core_mask
                else:
                    self._command = "nonce"
                    self._small_core_id = self._nonce >> (32 - self._core_bits - core_small_core_bits) & core_small_core_mask
                    self._midstates = self._chipadd
            reg_name_or_address = get_reg_name(self.bm_asic, self._regadd) if self._command == "read_register" or self._command == "write_register" or self._command == "respond" else ""
            reg_value_raw = f"0x{self._regval:08X}" if self._command == "write_register" or self._command == "respond" else ""
            ...
            if reg_name_or_address == "pll0_parameter" or reg_name_or_address == "pll1_parameter" or reg_name_or_address == "pll2_parameter" or reg_name_or_address == "pll3_parameter":
                ...
                if reg_name_or_address == "pll3_parameter" and self.bm_asic == "BM1397":
                    self._pll_uart_freq = freq
                if reg_name_or_address == "pll1_parameter" and self.bm_asic == "BM1366":
                    self._pll_uart_freq = freq
                reg_value = f"{freq} MHz"
            elif reg_name_or_address == "fast_uart_configuration":
                if self.bm_asic == "BM1397":
                    self._pll_uart_div4 = (self._regval >> 24) & 0b1111
                if self.bm_asic == "BM1366":
                ...
                if reg_name_or_address == "core_register_control":
                    ...
                    if (self._regval >> 31) & 0b1 == 0b1:
                        reg_value = f"WR@{get_core_reg_name(self.bm_asic, core_reg_id)}=0x{core_reg_val:02X}"
                    else:
                        reg_value = f"RD@{get_core_reg_name(self.bm_asic, core_reg_id)}"
                else:
            ...
            return AnalyzerFrame(analyzer_frame_type, self._start_of_frame, frame.end_time, {
                ...
                'chip': "" if self._command == "nonce" or self._command == "nonce_ext" else f"{self._chipadd}" if self._all == "ONE" else "ALL",
                'register': reg_name_or_address,
                'value': reg_value,
                'value_raw': reg_value_raw,
                'version_bits': f"0x{self._version_bits:04X}" if self._command == "nonce_ext" else "",
                'core_id': f"{core_id}" if (reg_name_or_address == "core_register_control" or reg_name_or_address == "core_register_status" or self._command == "nonce" or self._command == "nonce_ext") else "",
                'job_id': f"{self._job_id}" if (self._command == "work" or self._command == "nonce" or self._command == "nonce_ext") else "",
                'nonce': f"0x{self._nonce:08X}" if (self._command == "nonce" or self._command == "nonce_ext") else "",
                'small_core_id': f"{self._small_core_id}" if (self._command == "nonce" or self._command == "nonce_ext") else "",
